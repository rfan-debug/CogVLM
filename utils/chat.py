...
def process_image(text, text_processor, img_processor, image=None):
    ...
    pattern = (text_processor.tokenizer.boi + r"(.*?)" + text_processor.tokenizer.eoi).replace('[', r'\[').replace(']',
                                                                                                                   r'\]')
...
def chat(image_path,
    ...
    is_image_mode = image_path or (type(image) is not tuple and image is not None) or (
                type(image) is tuple and image != (None, None)) or force_pil_image is not None
    if not history:
        history = []
    if is_image_mode and not force_pil_image:
        prompt = "{}{}{}".format(text_processor.tokenizer.boi, image_path if image_path else "",
    ...
    strategy = BaseStrategy(temperature=temperature, top_p=top_p, top_k=top_k,
                            end_tokens=[text_processor.tokenizer.eos_token_id],
                            invalid_slices=invalid_slices, repetition_penalty=repetition_penalty)
    get_func = text_processor.get_func(inputs, **inputs_dic) if hasattr(text_processor,
                                                                        'get_func') else get_masks_and_position_ids_default
